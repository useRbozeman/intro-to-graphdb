...
query <- paste0(query, ", (sick:Health {status: 'sick'})")
query <- paste0(query, ", (healthy:Health {status: 'healthy'})")
query <- paste0(query, ", (unknown:Health {status: 'unknown'})")

d <- readLines("./data/graph3-health.csv")

for (i in 1:length(d))
{
    a <- as.character(unlist(read.csv(text = d[i], head = FALSE, stringsAsFactors = FALSE)))
    query <- paste0(query, ", (", a[1], ")-[:health]->(", a[2], ")")
}
